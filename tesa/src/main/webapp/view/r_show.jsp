<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<head>
<link rel="stylesheet" type="text/css" href="../css/r_style.css">
</head>
	<!-- Title Page -->
	<section class="bg-title-page p-t-130 p-b-140 flex-col-c-m"
		style="background-image: url(../images/r_main.jpg);">
		<h5 style="margin-bottom: 2%" class="l-text3 t-center">공연</h5>
		<h5 class="l-text4 t-center">공연과 함께 더 재미있는 하루를 만들어요!</h5>
	</section>
	<!-- tq -->
	<!-- Cart -->
	<section class="cart bgwhite p-t-50 p-b-100">
		<div class="container">
			<div class="flex-w flex-m w-full-sm con-date">
				<div class="size10 trans-0-4 m-t-10 m-b-10">
					<!-- Button -->
					<button
						class="flex-c-m sizefullb con-tc con-rad hov1 s-text1 trans-0-4">
						10/10</button>
				</div>
				<div class="size10 trans-0-4 m-t-10 m-b-10">
					<!-- Button -->
					<button
						class="flex-c-m sizefullb con-tc con-rad hov1 s-text1 trans-0-4">
						10/11</button>
				</div>
				<div class="size10 trans-0-4 m-t-10 m-b-10">
					<!-- Button -->
					<button
						class="flex-c-m sizefullb con-tc con-rad hov1 s-text1 trans-0-4">
						10/12</button>
				</div>
				<div class="size10 trans-0-4 m-t-10 m-b-10">
					<!-- Button -->
					<button
						class="flex-c-m sizefullb con-tc con-rad hov1 s-text1 trans-0-4">
						10/13</button>
				</div>
				<div class="size10 trans-0-4 m-t-10 m-b-10">
					<!-- Button -->
					<button
						class="flex-c-m sizefullb con-tc con-rad hov1 s-text1 trans-0-4">
						10/14</button>
				</div>
				<div class="size10 trans-0-4 m-t-10 m-b-10">
					<!-- Button -->
					<button
						class="flex-c-m sizefullb con-tc con-rad hov1 s-text1 trans-0-4">
						10/15</button>
				</div>
			</div>
			<!-- sub -->
			<div class="row">
			<ul class="con-items t-center">
			<li class="con-item1">
				<div class="con-sub"><a href="#">전체</a><a href="#">스테이지</a><a href="#">페스티벌</a><a href="#">기타공연</a></div>
				</li>
				</ul>
			</div>
			
			<!-- list -->
			<div class="row">
				<ul class="con-items">
					<li class="con-item">
						<p class="con-img"><img src="../images/rs_1.jpg"></p>
						<div class="con-text">
							<p class="con-title">드라큘라</p>
							<p class="con-content">무서워</p>
							<div class="con-times p-t-20">
							<p><span class="con-subject">공연 시간</span><span class="con-time">10:00 ~ 11:00</span></p>
							</div>
							<div class="con-times">
								<p><span class="con-subject">공연 장소</span><span class="con-time">집안</span></p>
							</div>
			<div class="flex-w flex-m w-full-sm m-t-10 m-b-10 btnw">
					<!-- Button -->
					<button
						class="flex-c-m sizefullbb con-tc con-rad2 hov1 s-text1 trans-0-4" onclick="r_re()">
						좌석예약</button>
					<!-- Button -->
					<button
						class="flex-c-m sizefullbb con-tc con-rad2 hov1 s-text1 trans-0-4 btn2">
						참가신청</button>
						</div>
							<div class="con-hash">
							<p>#<a>드라큘라</a>&nbsp;#<a>응 노잼</a></p>
							</div>
						</div>
						<!-- <a href="javascript:void(0)" class="con-detail t-center text-white">상세보기</a> -->
						<a href="r_detail.jsp" class="con-detail t-center text-white">상세보기</a>
					</li>
				</ul>
			</div>
			<div>※위 일정은 공연 사정에 따라 변경 또는 취소될 수 있습니다.</div>
			</div>
	</section>
